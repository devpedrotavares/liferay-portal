...
public class ObjectEntryLocalServiceImpl
	...
	private Iterable<Object> _getIterable(Object value) {
		...
		try {
			return _jsonFactory.createJSONArray(String.valueOf(value));
		}
	...
	private void _validateListTypeEntryKey(
		...
		if ((listTypeEntry == null) &&
			(!listTypeEntryKey.isEmpty() || objectField.isRequired())) {
			...
			if (objectField.compareBusinessType(
				...
				for (Object listTypeEntryKey : _getIterable(entry.getValue())) {
					_validateListTypeEntryKey(
						entry,
						GetterUtil.getString(String.valueOf(listTypeEntryKey)),
						objectField);
				}
			}
			else {
				_validateListTypeEntryKey(
					entry,
					GetterUtil.getString(String.valueOf(entry.getValue())),
					objectField);
