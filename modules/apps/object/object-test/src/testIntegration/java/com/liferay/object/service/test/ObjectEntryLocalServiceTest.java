...
public class ObjectEntryLocalServiceTest {
	...
	public void testAddObjectEntryWithAttachmentObjectField() throws Exception {
		...
		Assert.assertNotEquals(tempFileEntryId, values.get("upload"));

		DLFileEntry persistedFileEntry = _dlFileEntryLocalService.getFileEntry(
			(Long)values.get("upload"));

		Assert.assertEquals(
			_objectDefinition.getClassName(),
			persistedFileEntry.getClassName());
		Assert.assertEquals(
			objectEntry.getObjectEntryId(), persistedFileEntry.getClassPK());
		...
		long persistedFileEntryId1 = persistedFileEntry.getFileEntryId();
		...
		long persistedFileEntryId2 = (Long)values.get("upload");
